var MyApp = {
	start: function(){
		...
		var sliderMod = this.sb.startModule({
			id : "SliderContainer",
			module : Silder,
			config : {
				data : {
		...
		var imageMod = this.sb.startModule({
		...
		var pipe1 = new Pipe({
			pipeIn:{
				ModObj : sliderMod,
				source: "SliderValueChange"
			},
			pipeOut:{
				ModObj : imageMod,
				source: "ImageSrcChange"
			},
			formatter: ImageIdtoImageURLConverter
		});
...
var ImageBox = {
	start: function(){
		...
		this.sb.subscribe("ImageSrcChange",function(src){
...
var ImageIdtoImageURLConverter = function(id){
	var imageScrArray = 
